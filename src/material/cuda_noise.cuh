...
#define EPSILON 0.000000001f
	...
	__host__ __device__ unsigned int hash(unsigned int seed)
	{
	...
	__host__ __device__ float dot(float g[3], float x, float y, float z) {
	...
	__host__ __device__ unsigned char calcPerm12(int p)
	{
	...
	namespace cpuNoise {
		float cpuGradMap[16][3] = { { 1.0f, 1.0f, 0.0f },{ -1.0f, 1.0f, 0.0f },{ 1.0f, -1.0f, 0.0f },{ -1.0f, -1.0f, 0.0f },
	{ 1.0f, 0.0f, 1.0f },{ -1.0f, 0.0f, 1.0f },{ 1.0f, 0.0f, -1.0f },{ -1.0f, 0.0f, -1.0f },
	{ 0.0f, 1.0f, 1.0f },{ 0.0f, -1.0f, 1.0f },{ 0.0f, 1.0f, -1.0f },{ 0.0f, -1.0f, -1.0f } };
		__host__ float simplexNoise(Vector pos, float scale, int seed)
		{
			float xin = pos.x * scale;
			float yin = pos.y * scale;
			float zin = pos.z * scale;
			...
			if (t0 < 0.0f) n0 = 0.0f;
				...
				t0 *= t0;
				n0 = t0 * t0 * dot(cpuGradMap[gi0], x0, y0, z0);
			}
			float t1 = 0.6f - x1 * x1 - y1 * y1 - z1 * z1;
			if (t1 < 0.0f) n1 = 0.0f;
			else {
				t1 *= t1;
				n1 = t1 * t1 * dot(cpuGradMap[gi1], x1, y1, z1);
			}
			float t2 = 0.6f - x2 * x2 - y2 * y2 - z2 * z2;
			if (t2 < 0.0f) n2 = 0.0f;
			else {
				t2 *= t2;
				n2 = t2 * t2 * dot(cpuGradMap[gi2], x2, y2, z2);
			}
			float t3 = 0.6f - x3 * x3 - y3 * y3 - z3 * z3;
			if (t3 < 0.0f) n3 = 0.0f;
			else {
				t3 *= t3;
				n3 = t3 * t3 * dot(cpuGradMap[gi3], x3, y3, z3);
			}
